...
import 'package:f_local_database_hive_template/domain/entities/random_user.dart';
...
import 'package:f_local_database_hive_template/ui/controllers/user_controller.dart';
import 'package:f_local_database_hive_template/ui/pages/user_list_page.dart';
...
void main() {
  ...
  testWidgets('Add one, delete with swipe', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: mockNetworkImagesFor(() => const UserListPage()),
      ),
    );
    ...
    await tester.drag(
        find.byKey(const Key('userItem0')), const Offset(500.0, 0.0));
...
// Build our app and trigger a frame.
    ...
    expect(find.byKey(const Key('userItem0')), findsOneWidget);

    await tester.tap(find.byKey(const Key('deleteAllButton')));

    await tester.pumpAndSettle();

    expect(find.byKey(const Key('userItem')), findsNothing);
  });
}
